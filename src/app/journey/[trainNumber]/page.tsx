...
export default function JourneyPage() {
  ...
  // 1. Fetch Journey Metadata & Route GeoJSON
  const {
    data: routeData,
    ...
    queryKey: ["journeyRoute", trainNumber],
    queryFn: async () => {
  ...
  const {
    ...
    queryKey: ["journeyLive", trainNumber],
    queryFn: async () => {
  ...
  const { data: elevationData } = useQuery<{ data: ElevationData }>({
    queryKey: ["journeyElevation", trainNumber],
    queryFn: async () => {
  ...
  const { data: weatherData } = useQuery<{
    ...
    queryKey: ["journeyWeather", trainNumber],
    queryFn: async () => {
  ...
  const { data: geographyData } = useQuery<{ data: NearbyGeography }>({
    queryKey: ["journeyGeography", trainNumber],
    queryFn: async () => {
  ...
  if (isRouteError || !routeData?.data || !liveData?.data) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
        ...
        <h2 className="text-xl font-bold text-slate-900">Train Journey Not Found</h2>
        <p className="text-sm text-slate-500 max-w-sm mt-1">
          Could not find train details for #{trainNumber}. Please check the train number and try again.
        </p>
